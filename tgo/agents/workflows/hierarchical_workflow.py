...
class HierarchicalWorkflow(BaseWorkflow):
    ...
    def __init__(self, registry: Optional['AdapterRegistry'] = None):
        ...
        # Use registry if provided, otherwise TaskExecutor will use default
        if registry:
            self._task_executor = TaskExecutor(registry)
        else:
            from ..registry import get_registry
            self._task_executor = TaskExecutor(get_registry())
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    if hasattr(result, 'success'):
                        ...
                        self.log_agent_execution(expert.agent_id, getattr(result, 'success', False), getattr(result, 'error_message', "") or "")
                    else:
                        ...
                        self.log_agent_execution(expert.agent_id, False, f"Unknown result type: {type(result)}")
