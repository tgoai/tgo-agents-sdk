...
class ParallelWorkflow(BaseWorkflow):
    ...
    async def execute(
        ...
        try:
            ...
            all_agent_results:Dict[str, AgentExecutionResult] = {}
    ...
    def _create_agent_batches(
        ...
        batches:List[List[AgentInstance]] = []
    ...
    async def _execute_agent_batch(
        ...
        for agent in agents:
            ...
            agent_task = self._execute_single_agent_with_timeout(
            ...
            if isinstance(result, Exception):
                ...
                if hasattr(result, 'success'):
                    agent_results[agent.agent_id] = AgentExecutionResult(
                        ...
                        started_at=getattr(result, 'started_at', datetime.now(timezone.utc)),
                    ...
                    self.log_agent_execution(agent.agent_id, getattr(result, 'success', False), getattr(result, 'error_message', None))
                else:
                    now = datetime.now(timezone.utc)
                    agent_results[agent.agent_id] = AgentExecutionResult(
                        success=False,
    ...
    async def _execute_single_agent_with_timeout(
        ...
        try:
            ...
            return TaskResult(
                ...
                execution_time_ms=timeout_seconds * 1000 if timeout_seconds else 0,
                started_at=now,
                completed_at=now,
    ...
    def _synthesize_successful_results(
        ...
        for agent_id, result in successful_results:
            if result.result:
                ...
                elif result.result and hasattr(result.result, '__len__'):
                    # result.result is string-like
                    result_str = str(result.result)
                    synthesis["key_findings"].append(result_str[:200] + "..." if len(result_str) > 200 else result_str)
