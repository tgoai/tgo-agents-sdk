...
class CrewAIAdapter(BaseFrameworkAdapter):
    ...
    async def _process_crew_result(
        ...
        elif hasattr(crew_result, 'items'):
            # Handle dict-like objects safely
            result_data = {str(k): v for k, v in crew_result.items()}
        else:
    ...
    async def call_tool(
        ...
        # Note: context parameter available for future use
        start_time = datetime.now(timezone.utc)
    ...
    async def query_knowledge_base(
        ...
        try:
            ...
            if parameters:
                # Convert parameters to a safe format
