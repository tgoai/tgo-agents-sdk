...
class GoogleADKAdapter(BaseFrameworkAdapter):
    ...
    def __init__(self):
        ...
        self._run_configs: Dict[str, Any] = {}  # agent_id -> RunConfig
    ...
    async def _create_expert_agent(self, config: AgentConfig, tools: List[Any]) -> Any:
        ...
        print("config.agent_id---->",config.agent_id)
        ll = LlmAgent(
            name=config.agent_id,
            model=config.model,
            instruction=instructions,
    ...
    async def _execute_framework_task(
        ...
        try:
            ...
            # Execute with ADK
            result = await self._execute_adk_task(framework_agent, enhanced_task_input, run_config)
    ...
    async def _execute_adk_task(self, agent: Any, task_input: str, agentConfig: AgentConfig) -> Any:
        ...
        async for event in runner.run_async(user_id="test", session_id="test", new_message=user_content):
            # print(f"Event: {event.type}, Author: {event.author}") # Uncomment for detailed logging
            if event.is_final_response() and event.content and event.content.parts:
                # For output_schema, the content is the JSON string itself
                final_response_content = event.content.parts[0].text

        print(f"<<< Agent '{agent.name}' Response: {final_response_content}")
    ...
    async def _process_execution_result(
        ...
        if isinstance(adk_result, str):
            result_data = {"response": adk_result}
        elif isinstance(adk_result, dict):
            result_data = adk_result
        else:
            result_data = {"response": str(adk_result)}
        ...
        return AgentExecutionResult(
            ...
            started_at=start_time,
            completed_at=end_time,
            tool_calls=[],  # TODO: Extract tool calls from ADK result
